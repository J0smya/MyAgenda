...
const AYUDA: Record<string, { texto: string; opciones: string[] }> = {
  ...
  tareas: {
    ...
    opciones: ["Ver detalle de una tarea", "Eliminar una tarea", "Volver al inicio"],
  },
  ...
  tareas_ver: {
    ...
    opciones: ["Eliminar una tarea", "Volver a tareas", "Volver al inicio"],
  },

  tareas_eliminar: {
    texto: `**Eliminar una tarea:**\n\n1. Haz clic en la tarjeta de la tarea en el calendario\n2. En el panel emergente, pulsa **"Eliminar tarea"** (texto rojo al fondo)\n3. Confirma en el diálogo\n\nNota: Al eliminar una tarea también se eliminan sus notas y archivos asociados.`,
    opciones: ["Volver a tareas", "Volver al inicio"],
  },
  ...
  notas: {
    ...
    opciones: ["Nota adjunta a tarea", "Nota independiente", "Editar una nota", "Eliminar una nota", "Volver al inicio"],
  },
...
function detectarClave(msg: string): string {
  ...
  if (["inicio", "menu", "menú", "volver al inicio", "hola", "ayuda", "help", "start"].some(w => m === w || m.includes(w))) return "inicio";

  if (m.includes("ver detalle") || m.includes("ver tarea")) return "tareas_ver";
  if (m.includes("eliminar una tarea") || (m.includes("eliminar") && m.includes("tarea"))) return "tareas_eliminar";
  if (m.includes("volver a tareas")) return "tareas";
  if (m.includes("tarea") || m.includes("crear tarea")) return "tareas";

  if (m.includes("nota adjunta")) return "notas_adjunta";
  if (m.includes("nota independiente")) return "notas_independiente";
  if (m.includes("editar una nota") || (m.includes("editar") && m.includes("nota"))) return "notas_editar";
  if (m.includes("eliminar una nota") || (m.includes("eliminar") && m.includes("nota"))) return "notas_eliminar";
  if (m.includes("volver a notas")) return "notas";
  if (m.includes("nota") || m.includes("crear nota")) return "notas";

  if (m.includes("subir un archivo") || m.includes("subir")) return "archivos_subir";
  if (m.includes("descargar un archivo") || m.includes("descargar")) return "archivos_descargar";
  if (m.includes("eliminar un archivo") || (m.includes("eliminar") && m.includes("archivo"))) return "archivos_eliminar";
  if (m.includes("volver a archivos")) return "archivos";
  if (m.includes("archivo")) return "archivos";

  if (m.includes("recordatorio") || m.includes("alerta") || m.includes("notificacion") || m.includes("notificación")) return "recordatorios";
  if (m.includes("filtro") || m.includes("categoría") || m.includes("categoria")) return "filtros";
  if (m.includes("aria") || m.includes("asistente") || m.includes("sobre aria")) return "ia";
